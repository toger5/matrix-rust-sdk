...
use super::super::message::{ActionBody, Response};

pub struct Reply<Req, Resp> {
    request: ActionBody<Req, ()>,
    response: Sender<ActionBody<Req, Resp>>,
}

impl<Req, Resp> Reply<Req, Resp> {
    pub fn new(request: ActionBody<Req, ()>, response: Sender<ActionBody<Req, Resp>>) -> Self {
        Self { request, response }
    }

    pub fn reply(self, response: Resp) -> Result<(), Resp> {
        let message = ActionBody {
